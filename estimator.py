...
class Trainer:
        ...
        def model_fn(features, labels, mode):
            if mode == tf.estimator.ModeKeys.TRAIN:
                ...
                with tf.device('/gpu:0'):
                    ...
                    num_spk_per_batch = tf.constant(value=self.hparams.num_spk_per_batch, dtype=tf.int32, shape=[])

                    total_loss, total_loss_summary=self._cal_loss(norm_out, labels, num_utt_per_batch, num_spk_per_batch)
    ...
    def _cal_loss(self, norm_out, labels, num_utt_per_batch, num_spk_per_batch):
...
parser.add_argument('--data_types', nargs='+', default=['librispeech/libritest'])
...
# Train
parser.add_argument("--num_spk_per_batch", type=int, default= 32,
